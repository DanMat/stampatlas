...
function candidatesBand(){
  ...
  return \`<section class="band reveal" aria-labelledby="cand-h"><div class="wrap">
    ...
    <div class="grid g3" style="margin-top:18px">\${sample.map(c=>\`<div class="card"><div class="meta">\${dot("candidate","candidate")} \${esc(c.album)} · p\${esc(String(c.page))} · \${c.ordinal}/\${c.stampsOnPage}</div><p class="small" style="margin-top:8px">\${esc(c.reading||"Nothing legible yet — a closer photograph would help.")}</p></div>\`).join("")}</div>
...
function homePage(){ return frontispiece()+glanceBand()+candidatesBand()+issuersBand()+todayBand()+mapBand()+doorsBand()+recentBand(); }
