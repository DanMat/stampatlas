...
/* A drill-down link: an aggregate count → the actual candidate stamps behind it. */
const setHref=(dim,value)=>\`#/set/\${dim}/\${encodeURIComponent(value)}\`;
/* Inline drill-down link — subtle terra underline so entity text reads as clickable without shouting.
   Falls back to plain text when there's no value to link to. */
const sl=(dim,value,text)=>value?\`<a href="\${setHref(dim,value)}" style="color:inherit;text-decoration:underline;text-decoration-color:color-mix(in srgb,var(--terra) 55%,transparent);text-underline-offset:2px">\${text}</a>\`:text;
const candidateById=id=>(DATA.candidates||[]).find(c=>c.id===id)||null;
// Candidate aggregates — chapters light up from the machine's candidate corpus (hedged), not only
// from confirmed data. Computed once from DATA.candidates, which the page already ships.
let _candAgg=null;
function candAgg(){
  if(_candAgg) return _candAgg;
  const cs=DATA.candidates||[];
  const issM=new Map(), terrM=new Map(), thM=new Map(), denM=new Map(); let withDen=0, dated=0, monarch=0;
  for(const c of cs){
    const iname=c.resolvedIssuer||c.issuer;
    if(iname){ const e=issM.get(iname)||{name:iname,count:0,terr:new Map(),status:null,succeededBy:null};
      e.count++; if(c.territory) e.terr.set(c.territory,(e.terr.get(c.territory)||0)+1);
      if(!e.status&&c.issuerStatus) e.status=c.issuerStatus; if(!e.succeededBy&&c.succeededBy) e.succeededBy=c.succeededBy;
      issM.set(iname,e); }
    if(c.territory) terrM.set(c.territory,(terrM.get(c.territory)||0)+1);
    const t=c.theme; if(t&&!/^(unidentified|other)$/i.test(t)) thM.set(t,(thM.get(t)||0)+1);
    if(c.denomination){ withDen++; denM.set(c.denomination,(denM.get(c.denomination)||0)+1); }
    if(c.year) dated++;
    if(c.subthemes&&c.subthemes.monarch) monarch++;
  }
  const topTerr=e=>{ let best=null,bn=-1; e.terr.forEach((n,k)=>{ if(n>bn){bn=n;best=k;} }); return best; };
  const issuers=[...issM.values()].map(e=>({name:e.name,count:e.count,status:e.status,succeededBy:e.succeededBy,territory:topTerr(e)})).sort((a,b)=>b.count-a.count||a.name.localeCompare(b.name));
  const territories=[...terrM.entries()].map(([name,count])=>({name,count})).sort((a,b)=>b.count-a.count||a.name.localeCompare(b.name));
  const themes=[...thM.entries()].map(([name,count])=>({name,count})).sort((a,b)=>b.count-a.count||a.name.localeCompare(b.name));
  const denoms=[...denM.entries()].map(([value,count])=>({value,count})).sort((a,b)=>b.count-a.count);
...
const FRIENDLY = {
  ...
  countedStampCount:     {label:"Stamps counted",            d:"stamps on the photographed pages — a person's own count where they counted, the machine's page reading elsewhere; the vault's album totals. The confirmed figure is the hand-counted portion"},
...
function yearsTile(which){
  ...
  const label=which==="earliest"?"Earliest year of issue":"Latest year of issue";
  const e=y&&y[which];
  // Nothing dated at all — stay pending.
  if(!e||(e.confirmed==null&&e.candidate==null)) return which==="earliest"?pendingTile("earliestIssueYear"):\`<div class="perf"><div class="inner tile"><div class="n" style="font-weight:300;color:var(--ink-4)">—</div><div class="l">\${label}</div><div class="s"><span class="tag unknown">awaiting</span> identification</div></div></div>\`;
  // A person has confirmed a year — print it as the headline, name any candidate stretch beneath.
  if(e.confirmed!=null){
    const st=stampById(e.confirmedStampId);
...
function frontispiece(){
  ...
  const counted=metric("countedStampCount");
  const countedN=counted?((counted.value.confirmed||0)+(counted.value.candidate||0)):0; // effective total (vault rule)
  const handN=counted?(counted.value.confirmed||0):0;                                    // the hand-counted portion
  const np=(DATA.facts&&DATA.facts.namingProgress)||{namedByMachine:0,individualStamps:0,verifiedByPerson:0};
  const distinctRead=(DATA.facts&&DATA.facts.distinctReadings)||0;   // unique as read (copies collapsed)
  const itemsRead=np.namedByMachine||0;                              // pieces read (a block/sheet = one item)
  return \`<section class="front" id="top"><div class="wrap">
    <div>
      ...
      <p class="lede" style="margin-top:20px">One lifetime's collection of those windows, being charted page by page into an atlas of five Parts. \${countedN?\`<strong>\${fmt(countedN)} stamps counted so far</strong>\${named?\`, \${fmt(named)} of them named\`:\`, none named yet\`} — \${fmt(handN)} confirmed by hand, the rest the machine's page reading; the names come slowly, and the page says which is which.\`:\`<strong>Nothing has been counted yet.</strong> That is the honest first page of every atlas: come back and watch it fill in.\`}</p>
      <div class="actions">
      ...
      <div class="bigfacts">
        <div class="bf" title="Stamps on the photographed pages — a person's own count where they counted a page, the machine's page reading everywhere else; the same figure the private vault shows for each album.\${handN?\` \${fmt(handN)} of these are confirmed by hand.\`:''}"><div class="n">\${countedN?fmt(countedN):"—"}</div><div class="l">\${countedN?"counted":"awaiting the count"}</div></div>
        <div class="bf" title="Pieces the reader found in the photographs with look-alike copies collapsed into one — same issuer, year, value and subject. A fuzzy fingerprint and an estimate, never a verified duplicate count."><div class="n">\${distinctRead?fmt(distinctRead):"—"}</div><div class="l">distinct as read</div></div>
        <div class="bf" title="Every piece the reader found in the photographs, before de-duplication. A block or sheet counts as one item. A machine candidate reading, not yet confirmed by a person."><div class="n">\${itemsRead?fmt(itemsRead):"—"}</div><div class="l">items read</div></div>
      </div>
      <p class="small muted" style="margin-top:12px;max-width:56ch"><em>Counted</em> is a person's own count where a page was counted, the machine's page reading elsewhere — the collection's working total. The other two are the machine's <em>candidate</em> readings; as the reader improves year on year they should close on the counted total, and the gap is the work left to do.</p>
    </div>
...
function glanceBand(){
  const gnp=(DATA.facts&&DATA.facts.namingProgress)||{individualStamps:0};
  const indiv=gnp.individualStamps||0;   // every stamp, each sheet/block counted out — one section below the hero
  const indivLine=indiv?\`<p class="small" title="Every stamp once each sheet and block is counted out by its read contents. The reader's fullest tally and its loosest — empty printed sheet frames can be over-counted. A candidate count, not yet verified by a person." style="margin-top:14px;color:var(--ink-2);cursor:help"><strong>\${fmt(indiv)}</strong> individual stamps once every sheet and block is counted out <span class="small" style="color:var(--ink-3)">— the reader's fullest tally, still a candidate count and not yet verified by a person</span></p>\`:"";
...
function candidatePage(c){
  ...
  // "What we've found out" — sourced candidate facts (ADR-0054 D4). The machine asked; a named source
  // answered. Every value links back to its source; all hedged as unconfirmed, and honest that a
  // mis-read issuer carries the wrong history until a person corrects it.
  const kLabel={founded:"Founded",dissolved:"Dissolved","succeeded-by":"Succeeded by","preceded-by":"Preceded by",country:"Country",capital:"Capital","is-a":"In brief"};
  const kOrder={founded:1,dissolved:2,"succeeded-by":3,"preceded-by":4,country:5,capital:6,"is-a":7};
  // Generic class-descriptions add nothing next to the summary ("aspect of history", the philately
  // meta-article a term sometimes resolves to) — drop them, and show at most one 'is-a' at all.
  const kMeta=/postage stamps and postal history|aspect of|history of a country|^national flag$|^country\\b|sovereign state|^human$/i;
  const know=c.knowledge||[];
  const kSum=know.filter(k=>k.factType==="summary");
  let isa=0;
  const kFacts=know.filter(k=>k.factType!=="summary")
    .filter(k=>k.factType!=="is-a"?true:(kMeta.test(k.value)||isa++>=1?false:true))
    .sort((a,b)=>(kOrder[a.factType]||9)-(kOrder[b.factType]||9));
  const srcLink=k=>k.sourceUrl?\`<a href="\${esc(k.sourceUrl)}" target="_blank" rel="noopener nofollow">\${esc(k.source)}</a>\`:esc(k.source);
  const kCard=know.length?\`<div class="card" style="margin-top:18px">
      <h3 style="margin-top:0">What we've found out <span class="small" style="color:var(--ink-3);font-weight:400">· from open sources, unconfirmed</span></h3>
      \${kSum.map(k=>\`<p class="small" style="color:var(--ink-2);margin:6px 0 12px">\${esc(k.value)} <span style="color:var(--ink-3)">— \${srcLink(k)}</span></p>\`).join("")}
      \${kFacts.map(k=>row(kLabel[k.factType]||esc(k.factType), esc(k.value), srcLink(k))).join("")}
      <p class="small" style="color:var(--ink-3);border-top:1px solid var(--rule-soft);margin-top:10px;padding-top:10px">The machine asked the questions a collector would; each answer comes from the named source and is a candidate a person has not yet confirmed. Facts rest on the machine's reading of the issuer — a mis-read issuer carries the wrong history until corrected.</p>
    </div>\`:"";
  return \`\${crumbs}<section class="band"><div class="wrap" style="max-width:820px">
    ...
    <div class="grid g2 split" style="--split:1.4fr 1fr;align-items:start;margin-top:18px">
      <div class="card">
        ...
        \${issuerRow}
        \${row("Denomination", denomVal)}
        \${row("Year", c.year?esc(c.year):"")}
        \${row("Form", formVal)}
        \${row("Subjects", subjects)}
        \${row("Theme", c.theme?sl('theme',c.theme,esc(c.theme)):"")}
...
/* Client-side currency bucket — MUST mirror facts.ts parseFaceValue's buckets so the drill-down
   set matches the count it came from. Bucket only (no value), enough to filter. */
function candCurrency(c){
  const d=(c.denomination||"").trim(); if(!d) return null;
  const terr=(c.territory||"").toLowerCase(); const francLand=/france|monaco|switz|swiss|belg|luxemb/.test(terr);
  if(/€|\\beur/i.test(d)) return "€ euro";
  if(/£|\\bgbp/i.test(d)||/pence|penny|\\bp\\b/i.test(d)) return "£ sterling";
  if(/\\$|\\busd/i.test(d)||(/¢|cents?\\b|\\bc\\b/i.test(d)&&!francLand)) return "$ dollar";
  if(/\\bft\\b|forint/i.test(d)) return "Ft forint";
  if(/franc|\\bfr\\b|\\bfs\\b/i.test(d)) return "franc";
  if(/centime/i.test(d)||(/\\bc\\b/i.test(d)&&francLand)) return "franc";
...
function normDenom(d){
  if(!d) return "";
  let s=d.toLowerCase().trim().replace(/½/g,".5").replace(/¼/g,".25").replace(/¾/g,".75").replace(/⅓/g,".33").replace(/⅔/g,".67").replace(/\\s*\\+\\s*/g,"+");
  // Keep a semi-postal "80+20" distinct from "80"; otherwise canonicalise 8 = 8.0 = 8.00.
  const nm=s.match(/\\d[\\d.,+]*/); const raw=nm?nm[0].replace(/,/g,"."):null;
  const num=raw==null?null:(raw.includes("+")?raw:String(parseFloat(raw)));
...
const DIM_LABEL={territory:"Territory",theme:"Theme",person:"Person",monarch:"Monarch",sport:"Sport",animal:"Animal",mode:"Transport",cosmos:"Space",form:"Form",issuer:"Issuer",currency:"Currency",defunct:"No longer exists",album:"Album",page:"Page",denomination:"Denomination"};
const FACET_DIMS=["person","monarch","sport","animal","mode","cosmos"];
function setMatch(c,dim,value){
  if(dim==="territory") return c.territory===value;
  if(dim==="theme") return c.theme===value;
  if(FACET_DIMS.includes(dim)) return (c.subthemes||{})[dim]===value;
...
function setPage(dim,value){
  ...
  const backHref={territory:"#/territories",theme:"#/themes",person:"#/people",monarch:"#/monarchs",sport:"#/sports",animal:"#/animals",mode:"#/transport",cosmos:"#/space",form:"#/forms",issuer:"#/countries",currency:"#/money",defunct:"#/territories",album:"#/",page:pageParts?setHref('album',pageParts[0]):"#/",denomination:"#/money"}[dim]||"#/";
  ...
  return \`<section class="band"><div class="wrap"><span class="eyebrow"><a href="\${backHref}" style="color:inherit">\${esc(DIM_LABEL[dim]||dim)}</a>\${dim==="defunct"&&value==="*"?"":\` · \${esc(String(label))}\`}</span>
    ...
    <p class="lede" style="max-width:64ch"><strong>\${fmt(cands.length)}</strong> candidate stamp\${cands.length===1?"":"s"} \${leadPhrase}. Open one for the full read, its resolution, and its apparent duplicates.</p>
...
function issuersBand(){
  ...
  return \`<section class="band reveal" aria-labelledby="iss-h"><div class="wrap">
    ...
    <div class="grid g3" style="margin-top:16px">\${top.map(i=>\`<a class="card" href="\${setHref('issuer',i.name)}" style="display:flex;justify-content:space-between;align-items:baseline;gap:12px;color:inherit;text-decoration:none"><span>\${dot(i.confirmed>0?"confirmed":"candidate","")} \${esc(i.name)}</span><strong style="font-family:var(--ff-display);font-size:1.5em">\${fmt(i.candidate+i.confirmed)}</strong></a>\`).join("")}</div>
    \${CANDLEGEND}
  </div></section>\`;
}
...
function extinctCard(){
  ...
  const rows=list.map(d=>\`<li><a href="\${setHref('defunct',d.name)}" style="display:flex;justify-content:space-between;gap:12px;padding:7px 0;border-top:1px solid var(--rule-soft);color:inherit;text-decoration:none"><span>\${dot("candidate","")} <strong>\${esc(d.name)}</strong>\${d.succ?\` <span class="small" style="color:var(--ink-3)">→ \${esc(d.succ)}</span>\`:""}</span><span class="small">\${fmt(d.count)}</span></a></li>\`).join("");
...
function countriesPage(){
  ...
  const rows=iss.map(i=>{
    ...
    return \`<li><a href="\${setHref('issuer',i.name)}" style="display:flex;justify-content:space-between;gap:14px;padding:6px 0;border-bottom:1px solid var(--ink-4);color:inherit;text-decoration:none"><span>\${dot(i.confirmed>0?"confirmed":"candidate","")} \${esc(i.name)}\${terr}\${mark}</span><span class="small">\${i.confirmed?\`\${fmt(i.confirmed)} confirmed\${i.candidate?" · ":""}\`:""}\${i.candidate?\`\${fmt(i.candidate)}\${i.distinct<i.candidate?\` <span style="color:var(--ink-3)">· \${fmt(i.distinct)} distinct</span>\`:""}\`:""}</span></a></li>\`;
  }).join("");
...
function themesBand(){
  ...
  return \`<section class="band reveal" aria-labelledby="th-h"><div class="wrap">
    ...
    <div class="grid g3" style="margin-top:16px">\${top.map(t=>\`<a class="card" href="\${setHref('theme',t.name)}" style="display:flex;justify-content:space-between;align-items:baseline;gap:12px;color:inherit;text-decoration:none"><span>\${dot(t.confirmed>0?"confirmed":"candidate","")} \${esc(t.name)}</span><strong style="font-family:var(--ff-display);font-size:1.5em">\${fmt(t.candidate+t.confirmed)}</strong></a>\`).join("")}</div>
    \${CANDLEGEND}
  </div></section>\`;
}
function themesPage(){
  const th=(DATA.facts&&DATA.facts.themes)||[];
  const total=th.reduce((n,t)=>n+t.candidate+t.confirmed,0);
  const rows=th.map(t=>\`<li><a href="\${setHref('theme',t.name)}" style="display:flex;justify-content:space-between;gap:14px;padding:6px 0;border-bottom:1px solid var(--ink-4);color:inherit;text-decoration:none"><span>\${dot(t.confirmed>0?"confirmed":"candidate","")} \${esc(t.name)}</span><span class="small">\${t.confirmed?\`\${fmt(t.confirmed)} confirmed\${t.candidate?" · ":""}\`:""}\${t.candidate?\`\${fmt(t.candidate)}\${t.distinct<t.candidate?\` <span style="color:var(--ink-3)">· \${fmt(t.distinct)} distinct</span>\`:""}\`:""}</span></a></li>\`).join("");
...
function peopleBand(){
  const p=(DATA.facts&&DATA.facts.people)||[];
  if(!p.length) return "";
  const total=p.reduce((n,x)=>n+x.candidate+x.confirmed,0);
  const top=p.slice(0,15);
  return \`<section class="band reveal" aria-labelledby="ppl-h"><div class="wrap">
    <div class="band-head"><div><span class="eyebrow">By person</span><h2 id="ppl-h">The faces on the stamps</h2></div><a class="more" href="#/people">All \${fmt(p.length)} people →</a></div>
    <p class="small" style="max-width:66ch;margin-top:4px">The 'people' theme, subdivided into the individuals the machine named — from the \${fmt(total)} stamps it read as depicting a person. A candidate reading; a person confirms who's who over time. Anonymous and allegorical figures aren't named.</p>
    <div class="grid g3" style="margin-top:16px">\${top.map(x=>\`<a class="card" href="\${setHref('person',x.name)}" style="display:flex;justify-content:space-between;align-items:baseline;gap:12px;color:inherit;text-decoration:none"><span>\${dot("candidate","")} \${esc(x.name)}</span><strong style="font-family:var(--ff-display);font-size:1.5em">\${fmt(x.candidate+x.confirmed)}</strong></a>\`).join("")}</div>
    \${CANDLEGEND}
  </div></section>\`;
}
function peoplePage(){
  const p=(DATA.facts&&DATA.facts.people)||[];
  const total=p.reduce((n,x)=>n+x.candidate+x.confirmed,0);
  const rows=p.map(x=>\`<li><a href="\${setHref('person',x.name)}" style="display:flex;justify-content:space-between;gap:14px;padding:6px 0;border-bottom:1px solid var(--ink-4);color:inherit;text-decoration:none"><span>\${dot("candidate","")} \${esc(x.name)}</span><span class="small">\${fmt(x.candidate+x.confirmed)}\${x.distinct<x.candidate?\` <span style="color:var(--ink-3)">· \${fmt(x.distinct)} distinct</span>\`:""}</span></a></li>\`).join("");
  return \`<section class="band"><div class="wrap"><span class="eyebrow"><a href="\${chHref('themes')}" style="color:inherit">Themes</a> · By person</span>
    <h1 id="pageTitle" tabindex="-1" style="margin-top:8px">The people</h1>
    <p class="lede" style="max-width:66ch">\${fmt(p.length)} individuals the atlas named within the <a href="\${setHref('theme','people')}">people</a> theme, from \${fmt(total)} stamps it read as portraits. <strong>A candidate reading</strong> — each is only as good as the subject the machine read, and a person confirms who's depicted over time. Anonymous portraits and national allegories (Britannia, Marianne) are kept honest as unnamed, not forced into a name.</p>
    <ul style="list-style:none;padding:0;margin:20px 0 0;columns:2;column-gap:40px">\${rows||"<li class=small>No people named yet.</li>"}</ul>
    \${CANDLEGEND}
  </div></section>\`;
}
/* Config-driven sub-facets (ADR-0054): each subdivides a coarse theme into named values, rendered
   generically from DATA.facts.subfacets[scheme]. Add a facet = add a row here + run classify-facet. */
const SUBFACETS=[
  {scheme:"monarch", route:"monarchs", dim:"Monarch",  title:"The monarchs",  eye:"By monarch",       of:"royalty",   lead:"crowned heads named within the <a href='#/set/theme/royalty'>royalty</a> theme, from the stamps read as royal portraits"},
  {scheme:"sport",   route:"sports",   dim:"Sport",    title:"The sports",    eye:"By sport",          of:"sport",     lead:"sports named within the <a href='#/set/theme/sport'>sport</a> theme, from the stamps read as sporting subjects"},
  {scheme:"animal",  route:"animals",  dim:"Animal",   title:"The wildlife",  eye:"By animal group",   of:"wildlife",  lead:"animal groups named within the <a href='#/set/theme/wildlife'>wildlife</a> theme, from the stamps read as creatures"},
  {scheme:"mode",    route:"transport",dim:"Transport",title:"Ways to travel",eye:"By mode",           of:"transport", lead:"transport modes named within the <a href='#/set/theme/transport'>transport</a> theme, from the stamps read as vehicles"},
  {scheme:"cosmos",  route:"space",    dim:"Space",    title:"Space",         eye:"By space subject",  of:"space",     lead:"subjects named within the <a href='#/set/theme/space'>space</a> theme, from the stamps read as astronautics"},
];
const subfacetByScheme=s=>SUBFACETS.find(f=>f.scheme===s);
const subfacetByRoute=r=>SUBFACETS.find(f=>f.route===r);
const subfacetRows=s=>((DATA.facts&&DATA.facts.subfacets&&DATA.facts.subfacets[s])||[]);
function subfacetBand(cfg){
  const p=subfacetRows(cfg.scheme); if(!p.length) return "";
  const total=p.reduce((n,x)=>n+x.candidate+x.confirmed,0);
  const top=p.slice(0,12);
  return \`<section class="band reveal" aria-labelledby="sf-\${cfg.scheme}"><div class="wrap">
    <div class="band-head"><div><span class="eyebrow">\${esc(cfg.eye)}</span><h2 id="sf-\${cfg.scheme}">\${esc(cfg.title)}</h2></div><a class="more" href="#/\${cfg.route}">All \${fmt(p.length)} →</a></div>
    <p class="small" style="max-width:66ch;margin-top:4px">The '\${esc(cfg.of)}' theme, subdivided — \${cfg.lead}. A candidate reading, from \${fmt(total)} stamps; a person confirms it over time. Generic and unclear subjects aren't named.</p>
    <div class="grid g3" style="margin-top:16px">\${top.map(x=>\`<a class="card" href="\${setHref(cfg.scheme,x.name)}" style="display:flex;justify-content:space-between;align-items:baseline;gap:12px;color:inherit;text-decoration:none"><span>\${dot("candidate","")} \${esc(x.name)}</span><strong style="font-family:var(--ff-display);font-size:1.5em">\${fmt(x.candidate+x.confirmed)}</strong></a>\`).join("")}</div>
    \${CANDLEGEND}
  </div></section>\`;
}
function subfacetPage(cfg){
  const p=subfacetRows(cfg.scheme);
  const total=p.reduce((n,x)=>n+x.candidate+x.confirmed,0);
  const rows=p.map(x=>\`<li><a href="\${setHref(cfg.scheme,x.name)}" style="display:flex;justify-content:space-between;gap:14px;padding:6px 0;border-bottom:1px solid var(--ink-4);color:inherit;text-decoration:none"><span>\${dot("candidate","")} \${esc(x.name)}</span><span class="small">\${fmt(x.candidate+x.confirmed)}\${x.distinct<x.candidate?\` <span style="color:var(--ink-3)">· \${fmt(x.distinct)} distinct</span>\`:""}</span></a></li>\`).join("");
  return \`<section class="band"><div class="wrap"><span class="eyebrow">\${esc(cfg.eye)}</span>
    <h1 id="pageTitle" tabindex="-1" style="margin-top:8px">\${esc(cfg.title)}</h1>
    <p class="lede" style="max-width:66ch">\${fmt(p.length)} \${esc(cfg.dim.toLowerCase())} groups the atlas named — \${cfg.lead}. <strong>A candidate reading</strong>: each is only as good as the subject the machine read, and a person confirms it over time. Generic or unclear subjects are kept honest as unnamed.</p>
    <ul style="list-style:none;padding:0;margin:20px 0 0;columns:2;column-gap:40px">\${rows||"<li class=small>Nothing named yet.</li>"}</ul>
    \${CANDLEGEND}
  </div></section>\`;
}
/* One compact directory for the theme sub-facets — a link per facet, not a full band each, so the
   home page stays short and clicking a facet opens its own list (the big lists live on their pages). */
const FACET_NAV={people:"People",monarchs:"Monarchs",sports:"Sports",animals:"Wildlife",transport:"Transport",space:"Space"};
function facetsDirectoryBand(){
  const items=[];
  const ppl=(DATA.facts&&DATA.facts.people)||[];
  if(ppl.length) items.push({route:"people",label:"People",k:ppl.length,n:ppl.reduce((a,x)=>a+x.candidate+x.confirmed,0)});
  SUBFACETS.forEach(f=>{ const rows=subfacetRows(f.scheme); if(rows.length) items.push({route:f.route,label:FACET_NAV[f.route]||f.title,k:rows.length,n:rows.reduce((a,x)=>a+x.candidate+x.confirmed,0)}); });
  if(!items.length) return "";
  return \`<section class="band reveal" aria-labelledby="facdir-h"><div class="wrap">
    <div class="band-head"><div><span class="eyebrow">Who &amp; what</span><h2 id="facdir-h">Zoom into a theme</h2></div><span class="small muted italic">each opens its own list</span></div>
    <p class="small" style="max-width:66ch;margin-top:4px">The broad themes above, subdivided into named values — the people on the stamps, the monarchs, the sports, the wildlife, the ways things travelled, the reaches of space. Candidate readings; a person confirms them over time.</p>
    <div class="grid g3" style="margin-top:14px">\${items.map(it=>\`<a class="card" href="#/\${it.route}" style="display:flex;justify-content:space-between;align-items:baseline;gap:12px;color:inherit;text-decoration:none"><span>\${esc(it.label)} <span class="small" style="color:var(--ink-3)">· \${fmt(it.k)}</span></span><strong style="font-family:var(--ff-display);font-size:1.3em">\${fmt(it.n)}</strong></a>\`).join("")}</div>
  </div></section>\`;
}
function territoriesBand(){
  ...
  return \`<section class="band reveal" aria-labelledby="terr-h"><div class="wrap">
    ...
    <div class="grid g3" style="margin-top:16px">\${top.map(x=>\`<a class="card" href="\${setHref('territory',x.name)}" style="display:flex;justify-content:space-between;align-items:baseline;gap:12px;color:inherit;text-decoration:none"><span>\${dot("candidate","")} \${esc(x.name)}</span><strong style="font-family:var(--ff-display);font-size:1.5em">\${fmt(x.candidate+x.confirmed)}</strong></a>\`).join("")}</div>
    \${CANDLEGEND}
  </div></section>\`;
}
function territoriesPage(){
  ...
  const rows=t.map(x=>\`<li><a href="\${setHref('territory',x.name)}" style="display:flex;justify-content:space-between;gap:14px;padding:6px 0;border-bottom:1px solid var(--ink-4);color:inherit;text-decoration:none"><span>\${dot("candidate","")} \${esc(x.name)}</span><span class="small">\${fmt(x.candidate+x.confirmed)}\${x.distinct<x.candidate?\` <span style="color:var(--ink-3)">· \${fmt(x.distinct)} distinct</span>\`:""}</span></a></li>\`).join("");
...
function formsBand(){
  ...
  return \`<section class="band reveal" aria-labelledby="form-h"><div class="wrap">
    ...
    <div class="grid g3" style="margin-top:16px">\${top.map(x=>\`<a class="card" href="\${setHref('form',x.name)}" style="display:flex;justify-content:space-between;align-items:baseline;gap:12px;color:inherit;text-decoration:none"><span>\${dot("candidate","")} \${esc(formLabel(x.name))}</span><strong style="font-family:var(--ff-display);font-size:1.5em">\${fmt(x.candidate)}</strong></a>\`).join("")}</div>
    \${CANDLEGEND}
  </div></section>\`;
}
function formsPage(){
  ...
  const rows=f.map(x=>\`<li><a href="\${setHref('form',x.name)}" style="display:flex;justify-content:space-between;gap:14px;padding:6px 0;border-bottom:1px solid var(--ink-4);color:inherit;text-decoration:none"><span>\${dot("candidate","")} \${esc(formLabel(x.name))}</span><span class="small">\${fmt(x.candidate)}\${x.distinct<x.candidate?\` <span style="color:var(--ink-3)">· \${fmt(x.distinct)} distinct</span>\`:""}</span></a></li>\`).join("");
...
function moneyBand(){
  ...
  return \`<section class="band reveal" aria-labelledby="cur-h"><div class="wrap">
    ...
    <div class="grid g3" style="margin-top:16px">\${top.map(x=>\`<a class="card" href="\${setHref('currency',x.name)}" style="display:flex;justify-content:space-between;align-items:baseline;gap:12px;color:inherit;text-decoration:none"><span>\${dot("candidate","")} \${esc(x.name)} <span class="small" style="color:var(--ink-3)">· \${fmt(x.candidate)}</span></span><strong style="font-family:var(--ff-display);font-size:1.4em">\${x.valueTotal!=null?\`\${esc(x.unit)}\${fmt(Math.round(x.valueTotal))}\`:"—"}</strong></a>\`).join("")}</div>
    \${CANDLEGEND}
  </div></section>\`;
}
function moneyPage(){
  ...
  const rows=c.map(x=>\`<li><a href="\${setHref('currency',x.name)}" style="display:flex;justify-content:space-between;gap:14px;padding:6px 0;border-bottom:1px solid var(--ink-4);color:inherit;text-decoration:none"><span>\${dot("candidate","")} \${esc(x.name)}</span><span class="small">\${x.valueTotal!=null?\`<strong>\${esc(x.unit)}\${fmt(Math.round(x.valueTotal*100)/100)}</strong> · \`:""}\${fmt(x.candidate)} stamp\${x.candidate===1?"":"s"}</span></a></li>\`).join("");
...
function homePage(){ return frontispiece()+glanceBand()+candidatesBand()+issuersBand()+themesBand()+facetsDirectoryBand()+territoriesBand()+formsBand()+moneyBand()+todayBand()+mapBand()+doorsBand()+recentBand(); }
...
function timeCh(){
  ...
  <div class="grid" style="gap:12px">\${yearsTile("earliest")}\${yearsTile("latest")}<div class="card flat"><span class="eyebrow">Span</span><p class="small" style="margin:6px 0 0">\${(y.earliest.confirmed!=null&&y.latest.confirmed!=null)?\`Confirmed: <strong>\${y.earliest.confirmed}–\${y.latest.confirmed}</strong>, \${y.latest.confirmed-y.earliest.confirmed} years. \${(y.earliest.candidate!=null&&y.latest.candidate!=null)?\`If every candidate were right: \${y.earliest.candidate}–\${y.latest.candidate}. The atlas prints the first and names the second.\`:""}\`:(y.earliest.candidate!=null&&y.latest.candidate!=null)?\`The machine's candidate reach spans <strong>\${y.earliest.candidate}–\${y.latest.candidate}</strong>, \${y.latest.candidate-y.earliest.candidate} years — its dating, awaiting a person to confirm the ends.\`:"Awaiting a dated stamp."}</p></div></div></div>\`;
...
function issuersCh(){
  ...
  if(!conf.length){
    const A=candAgg();
    if(A.issuers.length){
      const rows=A.issuers.slice(0,150).map(i=>\`<tr data-key="\${esc(i.name)}"><td><strong>\${esc(i.name)}</strong></td><td>\${i.territory?\`<a href="\${setHref('territory',i.territory)}">\${esc(i.territory)}</a>\`:"—"}</td><td>\${i.status==="defunct"?\`<span class="tag defunct">no longer issues\${i.succeededBy?\` → \${esc(i.succeededBy)}\`:""}</span>\`:i.status==="current"?\`<span class="tag current">current</span>\`:\`<span class="tag unknown">unknown</span>\`}</td><td class="num">\${fmt(i.count)}</td></tr>\`).join("");
      const body=\`<div class="card"><span class="tag candidate">the machine's candidates — awaiting a person</span>
      <p class="small" style="margin-top:10px"><strong>\${fmt(A.issuers.length)}</strong> distinct issuers read across <strong>\${fmt(A.total)}</strong> candidate stamps\${A.extinct?\`, <strong>\${fmt(A.extinct)}</strong> of them places that no longer issue under the name printed\`:""}. An issuer here is the name the machine resolved from the inscription; a person has not confirmed these, so none is counted.</p>
      <div class="scroll-x"><table class="tbl"><thead><tr><th>Issuer (resolved)</th><th>Territory</th><th>Status</th><th style="text-align:right">Stamps</th></tr></thead><tbody>\${rows}</tbody></table></div>
      \${A.issuers.length>150?\`<p class="small muted" style="margin-top:10px">Showing the 150 most-read of \${fmt(A.issuers.length)}.</p>\`:""}</div>\`;
    ...
    return awaiting("issuers",2,"The issuers","Who printed the post — kingdoms, colonies, republics that lasted a decade, and the states that outlived them.","Every stamp is issued by somewhere. Once identification runs, this chapter counts the distinct issuers, and how many of them are places that no longer exist.",{pending:["distinctIssuerCount","extinctIssuerCount"],src:"assets/plate-issuers.jpg",cap:"Plate · Lost postal worlds"});
  }
...
function mapCh(){
  ...
  if(!iss.length){
    ...
    if(A.territories.length){
      const rows=A.territories.map(t=>\`<li data-key="\${esc(t.name)}" style="break-inside:avoid;border-top:1px solid var(--rule-soft);padding:8px 0;display:flex;justify-content:space-between;gap:10px"><a href="\${setHref('territory',t.name)}" style="text-decoration:none;color:var(--ink);font-weight:500">\${esc(t.name)}</a><span class="mono muted">\${fmt(t.count)}</span></li>\`).join("");
      const body=\`<div><span class="tag candidate">candidate territories — no coastlines drawn</span><p class="map-note" style="margin:12px 0 18px">The machine has placed candidate stamps in <strong>\${fmt(A.territories.length)}</strong> territories. A survey chart with pins waits on each territory being resolved to a seat with coordinates; until then the atlas lists them plainly rather than draw a map it cannot yet honestly plot. A place named is still a fact.</p>
      <div class="card place"><span class="eyebrow">Gazetteer · candidate</span><ul style="list-style:none;margin:10px 0 0;padding:0;column-width:220px;column-gap:28px">\${rows}</ul></div></div>\`;
...
function peopleCh(){
  ...
  // Candidate subdivision of the 'people' theme into named individuals (ADR-0054) — shown as a
  // ranking until confirmed named stamps exist; the awaiting plate only if nothing has been read.
  const facet=((DATA.facts&&DATA.facts.people)||[]).slice();
  if(!ppl.length && facet.length){
    const max=Math.max(...facet.map(p=>p.candidate+p.confirmed));
    const total=facet.reduce((n,p)=>n+p.candidate+p.confirmed,0);
    const bubbles=facet.slice(0,60).map(p=>{const n=p.candidate+p.confirmed;return \`<a class="bubble" href="\${setHref('person',p.name)}" style="--s:\${(52+74*(n/max)).toFixed(0)}px" title="\${esc(p.name)} · \${n}"><span class="t">\${esc(p.name)}</span><span class="c">\${n}</span></a>\`;}).join("");
    const body=\`<div class="card"><div class="bubbles">\${bubbles}</div><p class="small muted" style="text-align:center;margin:10px 0 0">\${fmt(facet.length)} people the machine named across \${fmt(total)} portrait readings — sized by how often each face recurs. A <strong>candidate</strong> reading; anonymous and allegorical figures aren't named. <a href="#/people">See the full list →</a></p></div>\`;
...
function themesCh(){
  ...
  if(!th.length){
    const A=candAgg();
    if(A.themes.length){
      const max=A.themes[0].count; const total=A.themes.reduce((n,t)=>n+t.count,0);
      const bubbles=A.themes.map(t=>\`<a class="bubble" href="\${setHref('theme',t.name)}" data-key="\${esc(t.name)}" style="--s:\${(52+74*(t.count/max)).toFixed(0)}px" title="\${esc(t.name)} · \${t.count}"><span class="t">\${esc(t.name)}</span><span class="c">\${fmt(t.count)}</span></a>\`).join("");
      const body=\`<div class="card"><div class="bubbles">\${bubbles}</div><p class="small muted" style="text-align:center;margin:10px 0 0">\${fmt(A.themes.length)} recurring themes the machine grouped across \${fmt(total)} candidate readings — sized by how often each recurs. A <strong>candidate</strong> grouping; a person hasn't confirmed the subjects yet.</p></div>\`;
      return chapter("themes",5,"The themes","What the collection keeps returning to — birds, ships, flight, the ends of wars.",body);
    }
    return awaiting("themes",5,"The themes","What the collection keeps returning to — birds, ships, flight, the ends of wars.","Once subjects are identified, the recurring themes surface here — and the honest absences, the subjects the collection never touches.",{src:"assets/plate-themes.jpg",cap:"Plate · A cabinet of subjects"});
  }
...
function dnaCh(){
  ...
  if(!storied().length){
    ...
    if(A.total){
      const lines=[
        \`<strong>\${fmt(A.total)}</strong> stamps have a candidate reading; none is confirmed, so every line here is the machine's, awaiting a person.\`,
        \`It resolved <strong>\${fmt(A.issuers.length)}</strong> distinct issuers across <strong>\${fmt(A.territories.length)}</strong> territories — <strong>\${fmt(A.extinct)}</strong> issuers no longer exist under the name printed.\`,
        \`<strong>\${fmt(A.themes.length)}</strong> recurring themes surfaced\${A.themes[0]?\`; the commonest is <strong>\${esc(A.themes[0].name)}</strong> (\${fmt(A.themes[0].count)} stamps)\`:""}.\`,
        \`<strong>\${fmt(A.dated)}</strong> carry a readable year, <strong>\${fmt(A.withDen)}</strong> a legible face value, and <strong>\${fmt(A.monarch)}</strong> a named monarch's head.\`,
      ];
      const body=\`<div class="card"><span class="tag candidate">a sketch, from \${fmt(A.total)} candidate readings</span><div style="display:grid;gap:14px;margin-top:14px">\${lines.map(l=>\`<p class="sentence" style="margin:0">\${l}</p>\`).join("")}</div><p class="small muted" style="margin:16px 0 0">Every sentence is computed from the machine's candidate readings, not written — and not yet confirmed. As stamps are named the sentences firm up.</p></div>\`;
      return chapter("dna",6,"The collection's DNA","The shape of the whole, distilled to a handful of honest sentences.",body);
    }
    return awaiting("dna",6,"The collection's DNA","The shape of the whole, distilled to a handful of honest sentences.","When there is enough read to summarise, this chapter states what the collection is — in plain, checkable lines drawn straight from the counts.",{cap:"Profile · pending",prompt:"The collection, distilled — once there is enough to distil."});
  }
...
function languageCh(){
  ...
  if(!st.length){
    const A=candAgg();
    if(A.denoms.length){
      const pct=Math.round(100*A.withDen/Math.max(1,A.total));
      const rows=A.denoms.slice(0,40).map(d=>\`<tr data-key="\${esc(d.value)}"><td>\${esc(d.value)}</td><td class="num">\${fmt(d.count)}</td></tr>\`).join("");
      const body=\`<div class="grid g2 split" style="--split:1fr 300px;align-items:start"><div class="card"><span class="tag candidate">face values as read — awaiting a person</span><h3 style="margin-top:10px">The commonest denominations</h3><p class="small">Read from <strong>\${fmt(A.withDen)}</strong> of \${fmt(A.total)} candidate stamps. Values are printed as the machine read them; currencies and decimal days resolve once a person confirms the issuers.</p><div class="scroll-x"><table class="tbl"><thead><tr><th>Face value (as read)</th><th style="text-align:right">Stamps</th></tr></thead><tbody>\${rows}</tbody></table></div>\${A.denoms.length>40?\`<p class="small muted" style="margin-top:10px">Showing the 40 commonest of \${fmt(A.denoms.length)} distinct values.</p>\`:""}</div>
      <div class="card flat"><p class="small"><strong>\${pct}%</strong> of candidate stamps carry a legible face value so far. The chart of currencies — annas and cents, marks by the thousand million — draws itself once face values are read across the whole collection, not just the commonest few.</p></div></div>\`;
    ...
    return awaiting("language",8,"Money &amp; language","Denominations and scripts — the day a currency went decimal, the alphabets the post was printed in.","Once the face values and inscriptions are read, this chapter charts the money and the writing systems across the collection.",{cap:"Denomination plate · pending"});
  }
...
function albumsCh(){
  ...
  if(!albums.length) body=\`<div class="grid g2 split" style="--split:1fr 320px;align-items:start"><div><span class="tag unknown">No album charted yet</span><p class="deck" style="margin:12px 0 0;font-style:normal;color:var(--ink)">The shelves are still empty. When the first album is photographed, it appears here — every position walked through in order, solid where a stamp was seen, dashed where the page was empty, hatched where nobody has looked yet.</p></div><div class="perf"><div class="inner" style="padding:10px">\${plate("era",{size:"full",cap:"First album · pending"})}</div></div></div>\`;
      ...
      return \`<div class="card" data-key="\${esc(a.physicalAlbumId)}"><a href="\${setHref('album',a.physicalAlbumId)}" style="display:block;color:inherit;text-decoration:none"><span class="tag current">Charted \${esc(niceDate(a.capturedOn))}</span><h3 style="margin-top:8px">\${esc(a.displayName||a.physicalAlbumId)} <span style="color:var(--ink-3);font-weight:400">→</span></h3><div class="mono small" style="color:var(--ink-3);margin-top:4px">\${esc(a.physicalAlbumId)}\${a.archetype?\` · \${esc(a.archetype)}\`:""}</div><p class="small" style="margin-top:10px">\${fmt(n)} position\${n===1?"":"s"} traversed · \${a.itemCount&&a.itemCount.total!=null?\`\${fmt(a.itemCount.total)} item\${a.itemCount.total===1?"":"s"} found\`:"items not yet counted"}</p></a>\${named.length?\`<p class="small" style="margin-top:6px;color:var(--ink-2)">\${named.length} named: \${named.map(s=>stampLink(s.id)).join(", ")}</p>\`:""}</div>\`; }).join("")+\`</div>\`;
...
function playCh(){
  ...
  if(!disc.length&&!dyk.length){
    const A=candAgg();
    if(A.total){
      const t0=A.territories[0], th0=A.themes[0], th1=A.themes[1], th2=A.themes[2];
      const discC=[
        t0?\`<div class="card hook surprise"><p class="q" style="margin-top:0">Which corner of the world fills the most pages?</p><p class="a"><a href="\${setHref('territory',t0.name)}">\${esc(t0.name)}</a> — \${fmt(t0.count)} candidate stamps, \${Math.round(100*t0.count/A.total)}% of everything read so far.</p><div class="grounded"><span>rests on</span><a href="\${setHref('territory',t0.name)}">\${esc(t0.name)}</a></div></div>\`:"",
        th0?\`<div class="card hook surprise"><p class="q" style="margin-top:0">What does the collection keep coming back to?</p><p class="a"><a href="\${setHref('theme',th0.name)}">\${esc(th0.name)}</a>, on \${fmt(th0.count)} stamps\${th1&&th2?\` — more than <a href="\${setHref('theme',th1.name)}">\${esc(th1.name)}</a> and <a href="\${setHref('theme',th2.name)}">\${esc(th2.name)}</a> together\`:""}.</p><div class="grounded"><span>rests on</span><a href="\${setHref('theme',th0.name)}">\${esc(th0.name)}</a></div></div>\`:"",
        \`<div class="card hook surprise"><p class="q" style="margin-top:0">How many of the issuers no longer exist?</p><p class="a"><strong>\${fmt(A.extinct)}</strong> of \${fmt(A.issuers.length)} — states that printed a stamp and then vanished under that name.</p><div class="grounded"><span>rests on</span><a href="\${chHref('issuers')}">the issuers</a></div></div>\`,
      ].filter(Boolean);
      const withK=(DATA.candidates||[]).filter(c=>c.knowledge&&c.knowledge.length);
      const step=Math.max(1,Math.floor(withK.length/6));
      const pick=[]; for(let i=0;i<withK.length&&pick.length<6;i+=step) pick.push(withK[i]);
      const dykC=pick.map(c=>{ const f=c.knowledge.find(k=>k.factType!=="summary")||c.knowledge[0]; const label=(c.subjects&&c.subjects[0])||c.resolvedIssuer||"a stamp"; return \`<div class="card hook"><p class="q" style="margin-top:0">\${esc(f.value)}</p><div class="grounded"><span>from \${esc(f.source||"a source")} on</span><a href="\${candHref(c.id)}">\${esc(label)}</a></div></div>\`; });
      const body=\`<div class="grid g2"><div class="grid" style="gap:12px"><span class="eyebrow">Discoveries · computed from the candidates</span>\${discC.join("")}</div>
      <div class="grid" style="gap:12px"><span class="eyebrow">Did you know · sourced</span>\${dykC.join("")||\`<div class="card"><p class="small muted">Sourced facts appear here as stamps are enriched.</p></div>\`}</div></div><p class="small muted" style="margin-top:14px">Nothing here is written by hand: a discovery is arithmetic over the machine's candidate readings, and a “did you know” is a sourced fact already sitting on a candidate stamp's page — both hedged, and awaiting a person.</p>\`;
...
function buildIndex(){
  ...
  if(DATA.facts&&(DATA.facts.people||[]).length) add("Pages","Page","People","the 'people' theme subdivided into named individuals","#/people",["people persons faces who portraits named individuals"]);
  SUBFACETS.forEach(f=>{ if(subfacetRows(f.scheme).length) add("Pages","Page",f.title,\`the '\${f.of}' theme subdivided by \${f.dim.toLowerCase()}\`,"#/"+f.route,[f.scheme+" "+f.of+" "+f.dim]); });
  ...
  (DATA.albums||[]).forEach(a=>{ const label=a.displayName||a.physicalAlbumId; add("Albums","Album",label,\`\${a.positionCount||0} position\${(a.positionCount||0)===1?"":"s"}\`,setHref('album',a.physicalAlbumId),[a.physicalAlbumId,"binder"]); });
...
function parseRoute(){
  ...
  if(seg[0]==="set"&&seg[1]) return {kind:"set",dim:seg[1],value:seg[2]?decodeURIComponent(seg.slice(2).join("/")):"*"};
  if(seg[0]==="countries"||seg[0]==="issuers") return {kind:"countries"};
  if(seg[0]==="themes") return {kind:"themes"};
  if(seg[0]==="people") return {kind:"peoplefacet"};
  { const f=subfacetByRoute(seg[0]); if(f) return {kind:"subfacet",scheme:f.scheme}; }
...
function navigate(){
  ...
  switch(r.kind){
    ...
    case "set":      html=setPage(r.dim,r.value); title=\`\${r.value==="*"?"Set":r.value} — Stamp Atlas\`; break;
    ...
    case "peoplefacet": html=peoplePage(); title="The people — Stamp Atlas"; break;
    case "subfacet": { const cfg=subfacetByScheme(r.scheme); html=subfacetPage(cfg); title=\`\${cfg.title} — Stamp Atlas\`; break; }
