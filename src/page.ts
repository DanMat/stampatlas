...
const STATUS={confirmed:"confirmed",candidate:"candidate",unknown:"not known",nm:"not measurable"};
...
function recentBand(){
  ...
  const right=\`<div class="card"><span class="eyebrow">The shelves</span><ul class="recent" style="margin-top:8px">\${albums.map(a=>\`<li style="grid-template-columns:1fr auto"><div><div class="ttl"><a href="\${chHref("albums")}">\${esc(a.label||a.physicalAlbumId)}</a></div><div class="meta">\${fmt(a.positionCount||0)} positions\${a.itemCount!=null?\` · \${fmt(a.itemCount)} items found\`:" · items not yet counted"}</div></div><span class="when">\${esc(niceDate(a.capturedAt))}</span></li>\`).join("")||\`<li class="small muted">No album charted yet.</li>\`}</ul></div>\`;
  return \`<section class="band reveal"><div class="wrap"><div class="grid g2">\${left}\${right}</div></div></section>\`;
}
function homePage(){ return frontispiece()+glanceBand()+todayBand()+mapBand()+doorsBand()+recentBand(); }
...
const stampLink=id=>{ const s=stampById(id); return s?\`<a href="\${stampHref(s.id)}">\${esc(s.short)}</a>\`:""; };
...
function glanceCh(){
  ...
  const notes=\`<div class="grid g2" style="margin-top:22px"><div class="card"><h3>Identified &amp; unresolved</h3>\${m?(function(){ const d=disp(m); return \`<p class="small"><strong>\${fmt(d.v.confirmed||0)}</strong> item\${(d.v.confirmed||0)===1?"":"s"} named\${d.v.candidate?\`, <strong>\${fmt(d.v.candidate)}</strong> still a machine's candidate\`:""}\${d.v.unknown?\`, <strong>\${fmt(d.v.unknown)}</strong> not yet looked at\`:""} — of \${fmt(d.pop)} found so far. The named ones have pages in <a href="#/stamps">the Register</a>; the unnamed wait in <a href="\${chHref("unknown")}">the Unknowns</a>.</p>\`; })():\`<p class="small">How many stamps the collection holds — and how many have been named — cannot be counted until the pages are read. Rather than show a zero that reads as emptiness, the atlas shows the promise: <em>awaiting \${esc((pendingOf("philatelicItemCount")||{}).blockedBy||"identification")}</em>.</p>\`}</div>
    <div class="card"><h3>Growth of the collection</h3><p class="small">\${positions===0?"Nothing has been photographed yet.":\`So far \${fmt(positions)} position\${positions===1?"":"s"} across \${fmt(albums.length)} album\${albums.length===1?"":"s"} \${albums.length===1?"has":"have"} been captured\${albums.length?\`, the latest on \${esc(niceDate(albums.map(a=>a.capturedAt).sort().pop()))}\`:""}.\`}</p></div></div>\`;
...
function albumsCh(){
  ...
  if(!albums.length) body=\`<div class="grid g2 split" style="--split:1fr 320px;align-items:start"><div><span class="tag unknown">No album charted yet</span><p class="deck" style="margin:12px 0 0;font-style:normal;color:var(--ink)">The shelves are still empty. When the first album is photographed, it appears here — every position walked through in order, solid where a stamp was seen, dashed where the page was empty, hatched where nobody has looked yet.</p></div><div class="perf"><div class="inner" style="padding:10px">\${plate("era",{size:"full",cap:"First album · pending"})}</div></div></div>\`;
      ...
      return \`<div class="card" data-key="\${esc(a.physicalAlbumId)}"><span class="tag current">Charted \${esc(niceDate(a.capturedAt))}</span><h3 style="margin-top:8px">\${esc(a.label||a.physicalAlbumId)}</h3><div class="mono small" style="color:var(--ink-3);margin-top:4px">\${esc(a.physicalAlbumId)}\${a.archetype?\` · \${esc(a.archetype.value)} \${dot(a.archetype.status)}\`:""}</div><p class="small" style="margin-top:10px">\${fmt(n)} position\${n===1?"":"s"} traversed · \${a.itemCount!=null?\`\${fmt(a.itemCount)} item\${a.itemCount===1?"":"s"} found\`:"items not yet counted"}\${named.length?\` · \${named.length} named: \${named.map(s=>stampLink(s.id)).join(", ")}\`:""}</p></div>\`; }).join("")+\`</div>\`;
...
function stampPage(s){
  ...
  const id=s.identity, ph=s.physical||{}; const iss=issuerById(id.issuerId); const set=setById(id.setId||s.connections.setId); const album=albumById(s.provenance.albumId);
  const year=yearOf(s);
  /* — head — */
  const crumbs=\`<div class="wrap st-crumbs"><a href="#/">Frontispiece</a><span>›</span><a href="#/stamps">Register</a><span>›</span><span>\${esc(s.short)}</span></div>\`;
  const head=\`<section class="st-head"><div class="wrap">
    ...
    <div class="st-idline"><span>as printed <b>\${esc(id.issuerAsPrinted.value)}</b></span><span>value <b>\${esc(denomOf(s)||"unread")}</b>\${id.denomination.status==="candidate"?" · candidate":""}</span>\${id.monarch&&id.monarch.value?\`<span>head <b>\${esc(id.monarch.value)}</b></span>\`:""}<span>album <b>\${esc(album?album.label.split(" · ")[0]:s.provenance.albumId)}</b>, page \${s.provenance.page}</span></div>
  </div></section>\`;
  /* — side: plate + facts — */
  ...
  const side=\`<aside class="st-side">
    ...
    <div class="card flat"><span class="eyebrow">Where it lives</span><div class="where" style="margin-top:8px">\${pageGlyph(s.provenance)}<div class="txt"><b>\${esc(album?album.label:s.provenance.albumId)}</b><br>page \${s.provenance.page}, position \${s.provenance.position} of \${s.provenance.positionsOnPage}<br><span class="muted">photographed \${esc(niceDate(s.provenance.capturedAt))} · identified \${esc(niceDate(s.provenance.identifiedAt))}</span></div></div></div>
  </aside>\`;
  /* — the ramble — */
  let ramble;
...
function buildIndex(){
  ...
  Array.from(yrs.keys()).sort().forEach(y=>{ const ss=yrs.get(y); add("Years","Year",String(y),ss.map(s=>s.short).join(", "),ss.length===1?stampHref(ss[0].id):chHref("time"),[String(Math.floor(y/10))+"0s decade"],ss.length===1?{}:{key:ss[0].id,sec:"time"}); });
  (DATA.sets||[]).forEach(t=>add("Sets","Set",t.name,\`\${t.valuesHeld} of \${t.valuesTotal} values\`,chHref("sets"),[String(t.year)],{key:t.id,sec:"sets"}));
  (DATA.albums||[]).forEach(a=>{ const label=a.label||a.physicalAlbumId; add("Albums","Album",label,\`\${a.positionCount||0} position\${(a.positionCount||0)===1?"":"s"}\`,chHref("albums"),[a.physicalAlbumId,"binder"],{key:a.physicalAlbumId,sec:"albums"}); });
