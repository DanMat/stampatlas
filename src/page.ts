...
function chapterStatus(id){
  const has={ glance:true, time:!!(DATA.facts&&DATA.facts.years), issuers:(DATA.issuers||[]).some(i=>i.status==="confirmed"), map:(DATA.issuers||[]).length>0, people:(DATA.people||[]).length>0||((DATA.facts&&DATA.facts.people)||[]).length>0,
