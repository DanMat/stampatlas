...
function themesPage(){
  const th=(DATA.facts&&DATA.facts.themes)||[];
  const total=th.reduce((n,t)=>n+t.candidate+t.confirmed,0);
  const rows=th.map(t=>\`<li style="display:flex;justify-content:space-between;gap:14px;padding:6px 0;border-bottom:1px solid var(--ink-4)"><span>\${dot(t.confirmed>0?"confirmed":"candidate","")} \${esc(t.name)}</span><span class="small">\${t.confirmed?\`\${fmt(t.confirmed)} confirmed\${t.candidate?" · ":""}\`:""}\${t.candidate?\`\${fmt(t.candidate)}\${t.distinct<t.candidate?\` <span style="color:var(--ink-3)">· \${fmt(t.distinct)} distinct</span>\`:""}\`:""}</span></li>\`).join("");
